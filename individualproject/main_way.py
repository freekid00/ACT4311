...
for a in [0.001,0.01,0.1,1]:
    for b in [1,3,5]:
        clf=SVC(kernel="rbf", gamma=b, coef0=1,C=a)
        clf.fit(train_data,train_label)
        print("The avarage AUC_ROC of the svc with C={} and gamma={} from 5-fold CV on test data is".format(a,b),roc_auc_score(test_label.tolist(),clf.predict(test_data).tolist()))
#%%
for c in [0.001,0.01,0.1,1]:
    clf=SVC(random_state=42,kernel='poly',degree=3,coef0=1,C=c)
    clf.fit(train_data,train_label)
    print("The AUC_ROC of the svc with C={} on test data is".format(c),roc_auc_score(test_label.tolist(),clf.predict(test_data).tolist()))
...
for a in [100,300,600,1000]:
    for b in [0.0001,0.001,0.01,0.1,1,10]:
        ...
        print("The reuslt AUC_ROC of the lightGBM with n_estimators={} and reg_lambda={} on test data is".format(a,b),
